...
SELECT COUNT(*) as treansaction_count FROM samples.bakehouse.sales_transactions
""")
...
SELECT DISTINCT paymentMethod FROM samples.bakehouse.sales_transactions ORDER BY paymentMethod 
""")
...
SELECT ROUND(AVG(totalPrice), 2) as avg_transaction_value FROM samples.bakehouse.sales_transactions
""")
...
avg_transaction_by_franchise_df = (transactions_df
    ...
    .agg(spark_round(avg(col("totalPrice")), 2).alias("avg_transaction_value"))  # Avg which column?
...
from pyspark.sql.functions import sum, count, avg, desc, date_format, col

franchise_metrics_df = (
    transactions_df
    .withColumn("payment_category", categorize_payment_udf(col("paymentMethod")))
    .withColumn("day_name", date_format(col("dateTime"), "EEEE"))
    .groupBy("franchiseID")
    .agg(
        sum(col("totalPrice")).alias("total_revenue"),
        count("*").alias("transaction_count"),
        avg(col("totalPrice")).alias("avg_transaction_value")
    )
