...
duplicates_df = (base_customers_df
    ...
    .withColumn("last_name",
        when(col("duplicate_id") % 2 == 0, upper(col("last_name"))).otherwise(col("last_name"))  # when(condition, upper(col("last_name"))).otherwise(col("last_name"))
    )
    ...
    .withColumn("email_address",
        when(col("duplicate_id") % 4 == 0, upper(col("email_address"))).otherwise(col("email_address"))  # when(condition, upper(col("email_address"))).otherwise(col("email_address"))
    )
